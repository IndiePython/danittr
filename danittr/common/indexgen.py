...
def generate_indices(index_data, index_map={}):
    ...
    if isinstance(index_data, Sequence): return index_data
    ...
    ## operation
        ...
        if index_data.get("is_eased", False):
            first_half = generate_indices(
              {"name": "easing", "index_data" : first_half},
              index_map
            )

            last_half = generate_indices(
              {"name":"easing", "index_data" : last_half},
              index_map
            )
    ...
    ## change indices by slicing existing ones
        ...
        indices = \
          generate_indices(
            index_data["index_data"], index_map)
    ...
    ## change indices by executing pre-defined operation
        ...
        if length % 2:
            ...
            for n, i in \
            enumerate(range(levels, -1, -1), 1):
                seq = [i] * n
                output = seq + output

            ## iterate from center to last index
            for n, i in \
            enumerate(range(levels, length), 1):
                if n == 1:
                    continue
                seq = [i] * n
                output += seq
            ...
            for n, i in \
            enumerate(range(left, -1, -1), 1):
                seq = [i] * n
                output = seq + output
            for n, i in \
            enumerate(range(right, length), 1):
                seq = [i] * n
                output += seq

        output = \
              list(map(indices.__getitem__, output))
    ...
    ## concatenate multiple sequences
        ...
        for sequence_data in index_data["sequences"]:
            output += \
                generate_indices(sequence_data, index_map)
