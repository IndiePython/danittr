...
def sign_factory(text, fg_color, bg_color,
                 signpost_height=0, direction=None):
    ...
    text_surf = render_text(text, font_size=22,
    ...
    if shape == "rectangular":
        ...
        sign_surf_width, sign_surf_height = \
                                       sign_surf.get_size()

        if not signpost_height:
            offset      = 5
            holder_surf = render_rect(
                                    sign_surf_width  + offset,
                                    sign_surf_height + offset)
            ...
            final_touch_rect = \
                Rect((sign_surf_width-2, sign_surf_height-2),
                     (4, 4))
            draw_rect(holder_surf, BLACK, final_touch_rect, 0)

        elif signpost_height:
            height_offset = sign_surf_height + 20
            signpost_offset_height = \
                             signpost_height + height_offset 
            pole_width = 18
            pole_surf  = render_rect(
                                     pole_width,
                                     signpost_offset_height)
            ...
            pole_x =   round(sign_surf_width / 2) \
                     - round(pole_width      / 2)
        ...
        if signpost_height:
            ...
            # just below the sign
            if direction in ("e", "w"):
                triangle = [( 0, height_offset - 10),
                            (40, height_offset - 10),
                            (40, height_offset + 30)]
            elif direction == "n":
                triangle = [( 0, height_offset +  5),
                            (40, height_offset +  5),
                            (40, height_offset + 45)]
            else:
                triangle = [( 0, height_offset - 25),
                            (40, height_offset - 25),
                            (40, height_offset + 15)]
