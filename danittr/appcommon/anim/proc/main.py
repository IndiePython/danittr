...
from ....common.pointgen import (
                               generate_points,
                               convert_items_to_tuples)
...
def process_anim_data(data):
    ...
    surf_map = get_filled_map(
                       {}, data,
                       "surface_map", "surface_map_post",
                       generate_surfaces)

    # also convert surface map values (lists) into tuples
    surf_map.update({
      key: tuple(value)
      for key, value in surf_map.items()
    })
    ...
    pos_map = get_filled_map(
                      {}, data,
                      "position_map", "position_map_post",
                      generate_points)
    ...
    for anim_name, anim_data in raw_anim_values.items():
        ...
        for node_name, node_data in anim_data.items():
            ...
            for value_type in value_types_names:
                ...
                # try retrieving data
                try: value_data = node_data[raw_data_key]

                # if absent, use default

                except KeyError:

                    node_anim_values[proc_data_key] = \
                                                  [default]
    ...
    for anim_name, anim_data in raw_timing_data.items():
        ...
        for node_name, node_timing_data in anim_data.items():
            ...
            ("surface_indices", "position_indices"):
                ...
                # try retrieving data
                try: timing_data = node_timing_data[key]

                # if absent, use default
                except KeyError:
                    node_timing_values[key] = \
                                           WalkingDeque([0])
                ...
                # otherwise, process and store values
                    ...
                    indices = generate_indices(
                                     timing_data, timing_map)

                    # create a walking deque with them
                    node_timing_values[key] = \
                                     WalkingDeque(indices)
    ...
    try: struct_map = data["structure_map"]
    ...
    for anim_name, anim_struct_data in structure.items():
        ...
        optional_order = get_tree_values(
                                    tree, "name", "children")
        ...
        for order in ("updating_order", "drawing_order"):
            ...
            try: anim_struct_data[order]

            except KeyError: anim_struct_data[order] = \
                                              optional_order
    ...
    metadata["proxy_anim_names"] = [
      anim_name
      for anim_name in structure
      if anim_name not in values_and_timing_keys
    ]
    ...
    try: geometry_map = data["geometry_map"]
