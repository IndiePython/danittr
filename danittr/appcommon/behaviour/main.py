...
from ...config import (
                   GAME_REFS,
                   LEVELS_DIR, SAVES_DIR, STATE_FILE_NAME)
...
def get_level(level_name):
    ...
    try: destination = GAME_REFS.destination
    except AttributeError: destination = None
    else: del GAME_REFS.destination

    msg = "Level named '{}' being loaded.".format(
                                             level_name)

    logger.debug(msg)


    level_filepath = join(SAVES_DIR, save_dir,
                          level_name)

    state_filepath = join(SAVES_DIR, save_dir,
                          STATE_FILE_NAME)

    try:
        new_level = \
        GameLevel(level_filepath, state_filepath, destination)
        ...
        new_level = \
           GameLevel(
              level_filepath, state_filepath, destination)
...
def get_custom_manager(manager_name):
    ...
    elif manager_name == "main_menu": return MainMenu()
