...
class ScrollBarrier(object):
    ...
    def __init__(self, rect, scroll_command):
        ...
        self.rect = rect
        self.image = render_rect(*self.rect.size,
                                          color=WHITE)
...
def set_scroll_handles(level):
    ...
    ## left
    left_rect = Rect((0, 0),
                     (vertical_handle_width, SCREEN_HEIGHT))
    ...
    right_barrier = ScrollBarrier(right_rect,
                                  level.scroll_right)
    
    ## upper
    upper_rect = Rect((0, 0),
                      (SCREEN_WIDTH, horiz_handle_height))
    ...
    lower_barrier = ScrollBarrier(lower_rect,
                                  level.scroll_down)

    ## Setups
    level.scroll_barriers = BlitterSet([left_barrier,
                                       right_barrier,
                                       upper_barrier,
                                       lower_barrier
                                       ])

    level.horizontal_barriers = BlitterSet([left_barrier,
                                           right_barrier
                                           ])

    level.vertical_barriers = BlitterSet([upper_barrier,
                                         lower_barrier
                                         ])
