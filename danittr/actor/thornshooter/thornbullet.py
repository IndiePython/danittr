...
class ThornBullet(BasicObject):
    ...
    def move(self):
        ...
        collided_platform = \
            self.level.onscreen_map["blocks"].collide_any(
                                                        self)
        ...
        hit_player = self.rect.colliderect(
                                    self.level.player.rect)

        if hit_player:

            self.reset_position()

            try: self.level.player.suffer_damage(self.power)
