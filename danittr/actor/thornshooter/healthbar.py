...
class HealthBar(BasicObject):
    ...
    def __init__(self, thorn_shooter):
        ...
        self.rect.midbottom = \
            unscroll_coordinates(
                        self.thorn_shooter.rect.midtop)
        self.rect.y += -10

        self.image.set_colorkey(ARBITRARY_COLORKEY)
        self.stencil = render_rect(
                                      *self.image.get_size(),
                                      ARBITRARY_COLORKEY)
    ...
    def update_width(self, amount=0):
        ...
        self.thorn_shooter.health  = \
            max(min(self.thorn_shooter.health,
                    self.max_health),
                    self.min_health)

        self.image.fill(self.fill_color)
        self.image.blit(self.stencil,
                        (self.thorn_shooter.health, 0))
