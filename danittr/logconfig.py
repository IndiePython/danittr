...
def json_serialize_traceback(exc_info):
    ...
    string_list = format_exception(exc_type, exc_value,
                                   exc_traceback)
...
class JsonLogFormatter(Formatter):
    ...
    def format(self, record):
        ...
        if record.exc_info != None:
            exc_info = json_serialize_traceback(
                                            record.exc_info)
        else:
...
latest_run_handler = \
              RotatingFileHandler(filename    = latest_filepath,
                                  mode        = 'a',
                                  maxBytes    =  0,
                                  backupCount = 10,
                                  encoding    = 'utf-8')
...
daily_run_handler = \
        TimedRotatingFileHandler(filename    = dailylog_path,
                                 when        = 'midnight',
                                 interval    =  1,
                                 backupCount = 16,
                                 encoding    = 'utf-8',
